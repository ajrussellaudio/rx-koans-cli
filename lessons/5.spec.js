...
describe("lesson 5", function() {
  it("instant merging", function() {
    ...
    number$.merge(letter$).subscribe(function(x) {
      result.push(x);
    });
  ...
  it("delayed merging", function(done) {
    ...
    number$.merge(letter$).subscribe(
      function(x) {
        result.push(x);
      }, // next
      function() {}, // error
      function() {
        assert.equal(result.join(","), "a,b,c,1,2,3");
        done();
      } // complete
    );
  });
  ...
  it("grouping with composition", function() {
    ...
    var grouped$ = number$.groupBy(function(n) {
      return n % 2;
    });
    ...
    odd$.subscribe(function(n) {
      odds.push(n);
    });
    even$.subscribe(function(n) {
      evens.push(n);
    });
  ...
  it("combining the latest", function(done) {
    ...
    var latest$ = Rx.Observable.combineLatest(number$, letter$, function(
      number,
      letter
    ) {
      return {
    ...
    latest$.take(1).subscribe(function(x) {
  ...
  it("combining the latest when a single observable fires", function(done) {
    ...
    var withLatest$ = slow$.withLatestFrom(fast$, function(slow, fast) {
      return {
        slow: slow,
        fast: fast
      };
    ...
    // Take the first element
    withLatest$.take(1).subscribe(function(x) {
